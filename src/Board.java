...
public class Board {
    ...
    boolean checkmate;
    boolean stalemate;
    Map<Piece, Collection<Move>> currentPossibleMoves;
...
//        Initialize board with pieces
    ...
    public void updateCurrentMoves() {
        ...
        for (Piece[] pieces : board) {
            for (Piece p : pieces) {
                if (p != null) {
                    Collection<Move> moves = p.getMoves(p.getXPos(), p.getYPos());
        ...
        currentPossibleMoves = checkBlockCheck(king, opposingPossibleMoves, currentPossibleMoves);
    ...
    public boolean selectPiece(int xPos, int yPos) {
        ...
        if (p.isWhite() == whiteTurn) {
            Collection<Move> moves = currentPossibleMoves.get(p);
            for (Move move : moves) {
                movesArray[move.getX()][move.getY()] = true;
            }
            movesArray[p.getXPos()][p.getYPos()] = true;
            return true;
        }
        return false;
    }
    ...
    private Collection<Move> removeVertHorizCollision(Collection<Move> moves, Piece p,
        ...
        for (Move move : moves) {
            if (!checkOutOfBounds(move.getX(), move.getY(), 
                ...
                if (move.getX() == p.getXPos() || move.getY() == p.getYPos()) {
                    rookMovesToRemove.add(move);
                }
            }
        }
    ...
    private Collection<Move> removeDiagCollision(Collection<Move> moves, Piece p, 
        ...
        for (Move move : moves) {
            if (!checkDiagBounds(move.getX(), move.getY(), p.getXPos(), p.getYPos(),
    ...
    private Collection<Move> checkPawnCapture(Piece p, Collection<Move> moves, 
                                              Piece[][] boardCopy) {
        int newX = 0;
        int newY = p.getYPos() + 1;
        int newY2 = p.getYPos() - 1;
        if (p.isWhite()) {
            newX = p.getXPos() + 1;
        } else {
            newX = p.getXPos() - 1;
    ...
    private Collection<Move> enPassant(Piece p, Collection<Move> moves, Piece[][] boardCopy) {
        if (p.getType().equals("Pawn")) {
            ...
            if (p.getXPos() == rowToCheck) {
                if (p.getYPos() + 1 < 8) {
                    Piece p1 = board[p.getXPos()][p.getYPos() + 1];
                    if (p1 != null) {
                        if (p1.getType().equals("Pawn")) {
                            if (p.isWhite() != p1.isWhite()) {
                                if (rowToCheck == 4) {
                                    moves.add(new Move(p.getXPos() + 1, p.getYPos() + 1,
                                            false, true, true));
                ...
                if (p.getYPos() - 1 >= 0) {
                    Piece p2 = board[p.getXPos()][p.getYPos() - 1];
                    if (p2 != null) {
    ...
    private Collection<Move> addCastle(Piece p, Collection<Move> moves, Piece[][] boardCopy) {
        if (p.getType().equals("King")) {
            ...
            if (castle == 1) {
                //  king side castle
                moves.add(new Move(p.getXPos(), p.getYPos() - 2, false, true));
            } else if (castle == 2) {
                // queen side castle
                moves.add(new Move(p.getXPos(), p.getYPos() + 2, false, true));
            } else if (castle == 3) {
    ...
    private boolean isCheck(Piece king, Map<Piece, Collection<Move>> opposingPossibleMoves) {
        for (Map.Entry<Piece, Collection<Move>> entry : opposingPossibleMoves.entrySet()) {
            ...
            for (Move move : moves) {
                if (move.getY() == king.getYPos() && move.getX() == king.getXPos()) {
                    return true;
                }
            }
        }
    ...
    public Map<Piece, Collection<Move>> checkMoveKing(Piece king, 
        ...
        if (moves != null) {
            for (Move move : moves) {
                ...
                boardCopy = movePiece(king.getXPos(), king.getYPos(), move, boardCopy);
    ...
    public Map<Piece, Collection<Move>> checkBlockCheck(Piece king, 
        ...
        for (Map.Entry<Piece, Collection<Move>> entry : currentPossibleMoves.entrySet()) {
            ...
            for (Move move : moves) {
                Piece[][] boardCopy = copyArray(board);
                boardCopy = movePiece(p.getXPos(), p.getYPos(), move, boardCopy);
                Map<Piece, Collection<Move>> updatedMoves = getUpdatedOpposingMoves(boardCopy);
                if (p.getType().equals("King")) {
